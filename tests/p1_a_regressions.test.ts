...
// ---------------------------------------------------------------- P1-006
test('P1-006: message router validates sender and allowlists asset URLs before cookie fetch', () => {
    const src = readSrc('content/messageRouter.ts');
    assert.ok(src.includes('sid !== ownId'), 'sender.id check missing');
...
// ---------------------------------------------------------------- P1-008 / P1-009
test('P1-008/P1-009: background rejects foreign senders; reloadGeminiTab only reloads Gemini tabs', () => {
    const src = readSrc('background/background.ts');
    assert.ok(src.includes('P1-008'), 'sender-rejection note missing');
    assert.ok(src.includes('sid !== ownId'), 'sender.id check missing');
