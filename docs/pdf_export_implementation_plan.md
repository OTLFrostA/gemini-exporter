...
共享模型是导出时的语义快照。最初可以由现有 `Conversation` 派生；将来归档 v1 可保存同一版本化语义，不能让 PDF renderer 的临时字段反向污染它。

```ts
interface ExportDocumentV1 {
  schemaVersion: 1;
  conversation: {
    key: ConversationKey; // 复合身份按长期路线图 F1 收口
    title: string;
    sourceUrl?: string;
    createdAt: number | null;
    updatedAt: number | null;
  };
  messages: ExportMessage[]; // 保留来源顺序
  resources: ResourceDescriptor[]; // 元数据；不含大块二进制
...
interface ExportMessage {
  ...
  timestamp: number | null; // 缺失时保持 null
  model?: string;
  blocks: BlockNode[];
  thoughts?: BlockNode[];
  citations?: Citation[];
  attachmentIds: string[];
  source?: { kind: 'rpc' | 'takeout' | 'dom' | 'legacy'; rawText?: string };
}
...
type BlockNode =
  ...
  | { type: 'table'; headers: InlineNode[][]; rows: InlineNode[][][]; aligns?: Array<'left' | 'center' | 'right'> }
  | { type: 'math'; latex: string; display: boolean }
  | { type: 'image'; resourceId: string; alt?: string; caption?: InlineNode[] }
  | { type: 'file'; resourceId: string }
  | { type: 'quote' | 'note'; blocks: BlockNode[] }
  | { type: 'unknown'; sourceType: string; fallbackText: string; raw?: unknown };
```

`InlineNode` 至少覆盖文本、粗体、斜体、行内代码、链接、换行和行内公式。正式实现用 TypeScript 判别联合，并明确最大嵌套深度、最大节点数与字符串长度，防止坏数据使解析或排版失控。`unknown` 的原始片段只作为诊断/后续重解析证据；公开 PDF/HTML 使用可读的 `fallbackText`。
...
`ResourceDescriptor` 保留资源 ID、所属消息、原始引用、展示名、MIME、声明尺寸、来源和获取状态。二进制存于有生命周期限制的 `ResolvedResourceStore`，由 `resourceId` 查找。规范化阶段合并同一消息中 `attachments/images/documents` 的重复对象，并把正文中的图片引用指向同一资源 ID。

资源状态至少为 `available`、`missing`、`failed`、`omitted`。PDF 和 HTML 都从同一状态决定占位内容；不能把 URL 存在等同于实体已保存。文件名和虚拟路径由打包层生成，拒绝 `..`、绝对路径和不可信协议。原附件是否另存到 ZIP/目录沿用 `includeAssets` 语义；产品界面需明确图片预览与原件保存的关系。
...
interface ExportArtifact {
  ...
  extension: 'html' | 'pdf';
  mimeType: string;
  companionResourceIds: string[]; // 需随产物另存的原件，由 Writer 解析 ID 并写入
  diagnostics: ExportDiagnostic[];
}
...
2. **Typst 适配器**：只把经验证的 AST 字段映射成 v8 所需 JSON/虚拟文件。文本始终走 JSON 数据而非拼进 `.typ` 源；URL 只允许预期协议。公式保留 LaTeX 原文；受控转换器能证明可转换时才产生 Typst math，失败时使用原文回退。v8 当前 `eval` 读取的 `typst` 字段绝不接受平台原文直通。
3. **视觉迁入**：把 v8 的 theme、components、render 文件作为版本化生产资源迁入仓库，保留独立的 demo/stress 夹具做视觉回归。迁入前记录原始文件版本与必要许可证；不得把机器上的字体名当成用户环境保证。生产模板读取适配器产物，不能依赖 fixture 路径。
...
HTML renderer 读取同一 Block AST 和 `ResourceDescriptor`，保留当前 Gemini 风格的用户气泡、助手正文、思考折叠、代码复制、深浅主题与打印样式。来源引用、代码、数学公式和表格由 AST 的相应节点输出；不再由 HTML 模板独立猜 Markdown 语法。
...
| D1 AST 契约 | TS 判别联合、来源格式、`unknown`/diagnostic、序列化版本及解析限制；真实旧数据适配 | 所有消息顺序与正文可核对；缺时间不伪造；未知块不丢 |
| D2 内容解析 | Markdown/Takeout HTML/plain 转 AST；附件引用合并、去重；重点覆盖嵌套列表、宽表、代码、公式 | AST 夹具与来源内容逐项比对；异常输入可回退 |
| D3 HTML 迁移 | AST → 现有 Gemini 风格 HTML；离线资源策略；保留交互 | 现有 HTML E2E、内容与视觉对照通过，旧入口再切换 |
...
| D6 v8 renderer | 模板资源迁入、AST → Typst 适配器、公式回退、引用/时间/附件占位 | demo/stress 与真实样本逐页检查；PDF 文本可提取且完整 |
