...
* **运行模式与 AI 执行铁律**：
  * **模式 A（无 AI 参与 / 离线自动化门禁）**：
    * 直接运行 `npm run test:live`，自动启用内置的高价值经典 5 轮数据集，保证离线自动化可重复性与零 Token 消耗。
  * **模式 B（有 AI 参与 / 协同开发与验收）**：
    * **必须由 AI 现场动态构思全新数据集**（严禁复用固定默认题目）：
    ...
    * 将构思的数据集保存为临时 JSON 并通过 `--dataset <path>` 传入执行全流程实跑。
  * **通用验收铁律（四大不可逾越标准）**：
