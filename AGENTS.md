...
* **运行模式与 AI 执行铁律**：
  ...
  * **通用验收铁律（四大不可逾越标准）**：
    ...
    3. **必须校验 Google Takeout 离线导入与全量历史合流**：测试流会自动读取预置的 `tests/fixtures/gemini_takeout_clean.zip`，检验离线导入初始提问前缀临时标题（`titleSource: takeout`），随后触发【全量拉取历史】(`btnDeepScan`) 地毯式分页同步所有云端历史，核实 Takeout 临时标题被在线 RPC 权威升级覆盖（`titleSource: rpc`）；
